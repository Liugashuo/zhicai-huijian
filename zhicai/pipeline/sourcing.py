...
class SourcingPipeline:
    def __init__(self, llm: LLMProvider | None = None, store: BenchmarkStore | None = None, driver: BrowserDriver | None = None) -> None:
        self.llm = llm
        self.store = store
        self.driver = driver

    def run(self, product_name: str, target_count: int = 5, dataset: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        state = StateManager()
        ...
        if self.store is not None:
            ...
            pid = self.store.upsert_product(product_name, category=analysis.get("category"), platform=(analysis.get("sites") or [None])[0])
            for item in benchmark.get("items", []):
                self.store.add_benchmark(pid, float(item.get("price", 0)), platform=(analysis.get("sites") or [None])[0], source_channel=item.get("source_channel", "DOM"), source_url=item.get("url"))
